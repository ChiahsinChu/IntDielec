...
from ..exts.toolbox.toolbox.plot.figure import FullCellFigure, Figure
...
class OrientEps(Eps):
    ...
    def make_plots(self):
        if not hasattr(self, "results"):
            self.results = load_dict(self.work_dir,
                                     "eps_data.%s" % self.data_fmt)
        ...
        # eps_ave
        lz = self.universe._trajectory.ts.dimensions[2] / 2 - self.results[
            "z_lo"]
        xlim = (0, lz)
        x = np.linspace(0, lz)
        ...
        ax.axhline(y=0., color=color, ls="--", alpha=0.5)
        ax.axhline(y=1., color=color, ls="--", alpha=0.5)
        ...
        const = constants.epsilon_0 / constants.micro * constants.centi / (
        ...
        figure.fig.savefig(
            os.path.join(work_dir, "differential_capacitance.png"))
